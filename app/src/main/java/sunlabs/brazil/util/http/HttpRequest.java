...
public class HttpRequest {
     ...
     * @throws IllegalArgumentException if <code>url</code> is not an "http:" URL.
     */
     ...
     * The host specified by the URL is <b>not</b> contacted at this time.
     *
     * @param    url A string representing a fully qualified "http:" URL.
     * @throws IllegalArgumentException if <code>url</code> is not a well-formed "http:" URL.
     */
     ...
     *
     * @param    headers The mime headers to be modified.
     * @param    response <code>true</code> to remove the point-to-point <b>response</b>
     ...
     *
     * @param    method The string for the HTTP method, or <code>null</code> to
     ...
     * is called.  Otherwise it will have no effect.
     *
     * @param    proxyHost The proxy that will handle the request, or <code>null</code>
     * to not use a proxy.
     * @param    proxyPort The port on the proxy, for the proxy request.  Ignored if
     ...
     * @throws IOException if there is a problem writing the HTTP request or reading
...
// System.out.println("receiving HTTP/0.9 response");
     ...
     * variable.
     *
     * @param    key The case-insensitive name of the response header.
...
class RecycleInputStream
    ...
    public RecycleInputStream(HttpRequest target, InputStream in) {
        super(in);
        this.target = target;
    }
...
class UnchunkingInputStream
    ...
    public UnchunkingInputStream(HttpRequest target) {
        super(target.in);
        this.target = target;
    }
